 ...
 *
 * Set NEXT_PUBLIC_SHEETS_URL in GitHub → Settings → Secrets → Actions
...
export async function postToSheet(payload: Record<string, unknown>): Promise<SheetResult> {
  ...
  try {
    // Apps Script requires no-cors for cross-origin POST; we use mode:'cors' and
    // the script sets the correct CORS headers via ContentService.
