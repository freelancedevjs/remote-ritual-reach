...
export default function ContactForm({ locale, accentClass = "bg-amber-600 hover:bg-amber-700" }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!name || !email || !subject || !message || !country) {
      setState("error"); setErrorMsg("Please fill in all required fields."); return
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      setState("error"); setErrorMsg("Please enter a valid email address."); return
    }
    if (message.length < 10) {
      setState("error"); setErrorMsg("Message must be at least 10 characters."); return
    }
  ...
  return (
    <form ref={formRef} onSubmit={handleSubmit} className="space-y-5 max-w-2xl mx-auto">
      ...
      <div className="grid md:grid-cols-2 gap-5">
        <div>
          <label className="block text-sm font-semibold text-stone-700 mb-1">Full Name <span className="text-red-500">*</span></label>
        ...
        <div>
          <label className="block text-sm font-semibold text-stone-700 mb-1">Email Address <span className="text-red-500">*</span></label>
      ...
      <div className="grid md:grid-cols-2 gap-5">
        <div>
          <label className="block text-sm font-semibold text-stone-700 mb-1">WhatsApp / Phone <span className="text-stone-400 font-normal">(optional)</span></label>
          <input name="phone" type="tel" placeholder="+1 555 000 0000"
            className="w-full border border-stone-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-stone-400 bg-white" />
        </div>
        <div>
          <label className="block text-sm font-semibold text-stone-700 mb-1">Country / Region <span className="text-red-500">*</span></label>
      ...
      <div>
        <label className="block text-sm font-semibold text-stone-700 mb-1">Subject <span className="text-red-500">*</span></label>
      ...
      <div>
        <label className="block text-sm font-semibold text-stone-700 mb-1">Related Sacred Place <span className="text-stone-400 font-normal">(if applicable)</span></label>
        <select name="relatedPlace"
          className="w-full border border-stone-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-stone-400 bg-white">
      ...
      <div>
        <label className="block text-sm font-semibold text-stone-700 mb-1">Your Message <span className="text-red-500">*</span></label>
      ...
      <div>
        <label className="block text-sm font-semibold text-stone-700 mb-1">How did you hear about us? <span className="text-stone-400 font-normal">(optional)</span></label>
        <select name="source"
          className="w-full border border-stone-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-stone-400 bg-white">
